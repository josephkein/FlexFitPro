...
<body class="flex items-center justify-center h-screen w-screen bg-gray-100 px-4">
    <div class="flex w-full max-w-md md:max-w-4xl shadow-lg rounded h-150">
        ...
        <form action="./actions/auth.php" method="post" class="bg-white flex-1 flex flex-col justify-center gap-6 w-full h-full px-6 py-6 md:py-2 rounded">
            ...
            <?php if ($invalid): ?>
                <span class="w-full p-2 text-lg text-red-500 bg-red-100 text-center rounded"><?= $invalid ?></span>
            <?php endif; ?>
            <div class="flex flex-col gap-2">
                <div class="relative flex flex-col gap-2">
                    ...
                    <input type="text" value="<?= $_SESSION['user'] ?? '' ?>" name="user" id="user" class="border rounded py-2 px-11 <?= $empty_user ? 'border-red-500 focus:outline-red-400' : 'focus:outline-violet-500' ?>">
                    <?php if ($empty_user): ?>
                        <span class="text-red-500"><?= $empty_user ?></span>
                    <?php endif; ?>
                </div>
                <div class="relative flex flex-col gap-2">
                    ...
                    <input type="password" value="<?= $_SESSION['pass'] ?? '' ?>" name="pass" id="pass" class="border rounded py-2 px-11 <?= $empty_pass ? 'border-red-500 focus:outline-red-400' : 'focus:outline-violet-500' ?>">
                    <?php if ($empty_pass): ?>
                        <span class="text-red-500"><?= $empty_pass ?></span>
                    <?php endif; ?>
            ...
            <button type="submit" class="py-2 bg-violet-600 text-white w-full text-lg rounded font-medium">Sign in</button>
