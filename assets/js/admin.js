        ...
        list.innerHTML += `
            <div class="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-violet-50 transition-colors">
                ...
                <div class="flex-1 min-w-0">
                    <div class="text-lg font-medium text-gray-800 truncate">${escHtml(d.name)}</div>
                ...
                <div class="text-right flex-shrink-0">
                    <div class="text-md text-gray-400 mb-1">${escHtml(formatted)}</div>
                    <span class="text-md font-medium px-2 py-0.5 rounded-full ${colors[d.payment_type].badge}">${escHtml(d.payment_type)}</span>
                </div>
            </div>
        `;
    });
